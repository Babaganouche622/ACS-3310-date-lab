...
function consecutiveDates(date, repeat, offset) {
  ...
  for (let i = 1; i < repeat; i++) {
    ...
    const nextTimestamp = lastDate.getTime() + (offset * 24 * 60 * 60 * 1000)
    const nextDate = new Date(nextTimestamp)
    dates.push(nextDate)
  }
  dates.forEach((date) => {console.log(formatDateString(date))})
}
...
function orderDates(dates) {
  ...
  sortedDates.forEach((date) => {console.log(formatDateString(date))})
}
...
function nextDate(dates) {
  ...
  console.log(`Your next apointment is ${nextDate.getDate() - today.getDate()} days from now: ${formatDateString(nextDate)}`)
}
...
function whensYourParty(date, year) {
  ...
  const yourBirthday = new Date(year, date.getMonth(), date.getDate())
  console.log(`Your birthday is on ${days[yourBirthday.getDay()]}day, ${yourBirthday.getDate()} of ${months[yourBirthday.getMonth()]} in ${year}}`)
}
